        ...
        <form action="{{  $user->hasRole('student') ? route('students.update', $user) : route('teachers.update', $user) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="md:grid grid-cols-12 gap-4">
                <div class="col-span-12 flex flex-col my-2">
                    ...
                    <input id="name" name="name" placeholder="{{ __('Your full name') }}" class="border border-gray-500 p-2 rounded bg-inherit w-full" value="{{ $user->name }}" />
                ...
                <div class="col-span-6 flex flex-col my-2">
                    ...
                    <input id="email" name="email" placeholder="{{ __('Your email address') }}" class="border border-gray-500 p-2 rounded bg-inherit w-full" value="{{ $user->email }}" />
                ...
                <div class="col-span-6 flex flex-col my-2">
                    ...
                    <input id="phone" name="phone" placeholder="{{ __('Your phone number') }}" class="border border-gray-500 p-2 rounded bg-inherit w-full" value="{{ $user->phone }}" />
                ...
                @if ($user->hasRole('student'))
                    <div class="col-span-6 flex flex-col my-2">
                        ...
                        <input type="date" id="birthday" name="birthday" placeholder="{{ __('Your birthday...') }}" class="border border-gray-500 p-2 rounded bg-inherit w-full" value="{{ $user->birthday }}" />
                ...
                @if (! $user->hasRole('student'))
                    ...
                    <div class="col-span-12 flex flex-col my-2">
                        ...
                        <textarea id="skills" name="skills" class="border border-gray-500 p-2 rounded bg-inherit w-full" rows="3" >{{ $user->skills }}</textarea>
                    </div>
                    <div class="col-span-12 flex flex-col my-2">
                        <label class="font-bold my-3">{{ __('What are your goals for this course?') }}</label>
                        <textarea id="description" name="description" class="border border-gray-500 p-2 rounded bg-inherit w-full" rows="5" >{{ $user->description }}</textarea>
                    </div>
                @endif
