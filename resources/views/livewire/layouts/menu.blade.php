<nav class="fixed lg:sticky inset-0 h-screen w-full lg:sidebar-expand lg:-mt-40 lg:ml-14 flex duration-250 transition-all z-40" :class="{'w-0 overflow-hidden lg:flex sidebar-collapse' : menuOpen == false}" >
    <div class="card mt-0" :class="{'grid' : menuOpen == false}">
        <div :class="{'flex float-right' : menuOpen == true, 'block text-center mt-5' : menuOpen == false}">
            <button role="button" class="text-2xl mx-3 text-dark" @click="menuOpen = !menuOpen">
                <p class="sr-only">Menu</p>
                <i class="fa fa-bars" aria-hidden="true"></i>
            </button>
        </div>
        <div class="card-body" :class="{'p-0' : menuOpen == false}">
            <aside class="md:w-3/6 lg:w-full bg-white beautify-scrollbar text-black" :class="{'overflow-scroll' : menuOpen == true, 'overflow-hidden' : menuOpen == false}">
                <div class="p-3">
                    <div class="text-center mb-10" :class="{'hidden' : menuOpen == false}">
                    ...
                    @isset ($menu)
                            ...
                            @elseif(!isset($menuItem['can']) || auth()->user()->can($menuItem['can']))
                                <div @click.outside="submenu = false" x-data="{
                                    ...
                                    @if (!isset($menuItem['submenu']))
                                        <a class="gap-2 p-3 px-4 my-0 rounded-xl hover:bg-red-100" href="{{route($menuItem['route'])}}"
                                           :class="{'bg-red-600 hover:bg-red-400 hover:bg-opacity-100 text-white' : {{Route::currentRouteName() == $menuItem['route'] ? '1' : '0'}},
                                           'flex' : menuOpen == true, 'text-center block' : menuOpen == false}" aria-label="{{$menuItem['text']}}"
                                            title="{{$menuItem['text']}}"
                                        >
                                        <span class="inline-block" :class="{'w-10' : menuOpen == true}">{!! $menuItem['icon'] !!}</span>
                                        <p x-show="menuOpen" class="text-lg font-medium">{{$menuItem['text']}}</p>
                                        </a>
                                    @else
                        ...
                        <form action="{{route('logout')}}" class="w-full" method="POST">
                            ...
                            <button href="" class="flex w-full text-dark gap-2 px-4 mt-7 text-center text-lg font-medium p-3 px-4 my-0 rounded-xl hover:bg-red-100">
                                ...
                                <p x-show="menuOpen" class="text-lg px-1">Log out</p>
                            </button>
                        </form>
                    @endif
                </div>
