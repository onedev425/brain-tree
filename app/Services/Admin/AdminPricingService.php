...
{
    public function getSoldCoursesOfTeacher($fromDate, $toDate, $search)
    {
        $courses = User::select('users.id', 'users.name', 'users.profile_photo_path')
            ...
            ->join('student_courses', function ($join) use ($fromDate, $toDate) {
                $join->on('student_courses.course_id', '=', 'courses.id')
                    ->whereRaw("LEFT(student_courses.created_at, 7) >= '$fromDate'")
                    ->whereRaw("LEFT(student_courses.created_at, 7) <= '$toDate'");
            })
            ->leftJoin('payment_salaries', function ($join) use ($fromDate, $toDate) {
    ...
    public function getPayoutHistories($fromDate, $toDate, $search)
    {
        ...
        if ($fromDate) {
            $courses = $payout_histories->whereDate('period', '>=', $fromDate);
        }

        // Apply toDate filter if provided
        if ($toDate) {
            $courses = $courses->whereDate('period', '<=', $toDate);
        }
