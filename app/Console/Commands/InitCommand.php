...
class InitCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'braintree:init';
     ...
     * @var string
     */
    protected $description = 'Easily install Braintree';
    ...
    public function setAppEnvironmentDetails()
    {
        ...
        $this->env = $this->choice(
            'What environment are you installing braintree on?',
            ['local', 'production'],
            $this->env
        );
    ...
    public function createSuperAdmin()
    {
        ...
        switch ($this->env) {
            case 'local':
                $this->info('Since you are trying out braintree locally, we have already created a super admin account for you. Check the docs on what these credentials are if unsure');
                break;
            default:
                $this->info('If you choose to not create a super admin now for some reason, you cen do so late by running php artisan braintree:create-super-admin');
                if ($this->confirm('Do you wish to create a super admin account now?', true)) {
                    $this->call('braintree:create-super-admin');
                } else {
                    $this->line('skipping...');
                }
                break;
